#!/bin/sh
# `mise run force` replaces the files rcm skipped because the account already had
...
assert_ok "mise run force succeeds" mise -C "$REPO" run force

for name in profile zprofile bashrc bash_profile; do
    dest=$HOME/.$name
    if [ "$(readlink "$dest" 2>/dev/null)" = "$REPO/rcm/$name" ]; then
        pass "mise run force links ~/.$name into the repository"
    else
        fail "mise run force links ~/.$name into the repository" \
            "$(ls -l "$dest" 2>&1)"
    fi
done
...
for shell in sh bash zsh; do
    ...
    assert_in_path "$shell: ~/bin is still on the PATH after mise run force" \
