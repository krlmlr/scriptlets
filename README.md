...
To install all scripts to `~/bin` (by creating symbolic links), clone the project *with submodules* and type `make` — or `./install`, which is the same thing.
...
The symlinks are declared, not generated:
there is no checked-in installer to regenerate whenever a file changes.
Only Python 3 and Git are needed at install time.

- [`install.conf.yaml`](install.conf.yaml): the single source of truth.
  It cleans stale links, creates `~/log`, and links everything into place.
  `home/bin/*` and `home/git/R/*` are matched by glob, so a new script needs no change here;
  the `dot-` prefixed files are listed one by one, because `dot-` is a convention of this repository and not of dotbot.
  The per-user layer is one glob over `personalized/$USER/*` — dotbot expands environment variables in both source and target,
  so a user without a `personalized/` directory simply matches nothing.
- [`install`](install): dotbot's entry point.
  It initialises the `dotbot/` submodule and runs dotbot against the config.
  Extra arguments are passed through, so `./install --dry-run --verbose` shows what would happen.
- [`dotbot/`](dotbot): dotbot itself, pinned as a Git submodule.
- [`Makefile`](Makefile): `make` runs `./install`, `make check` runs it as a dry run.
- [`bootstrap`](bootstrap): one-shot setup: clones the repo to `~/git/scriptlets` and runs `make`.

The `home/` and `personalized/` directories are unchanged;
dotbot reads them where they are.
