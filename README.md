...
```sh
homeshick clone krlmlr/scriptlets
cd ~/.homesick/repos/scriptlets && make
```

Or run the [`bootstrap`](bootstrap) script, which installs homeshick first if needed:
...
Installation is handled by [homeshick](https://github.com/andsens/homeshick).
It creates the symbolic links,
so there is no generated installer to keep in sync:
adding a file under `home/` and rerunning `make` is enough.

## Layout

A castle lives in `~/.homesick/repos/<name>`,
and everything under its [`home/`](home) directory is linked into `$HOME`.
The names are the real target names —
`home/.bashrc` is `~/.bashrc`, `home/bin/h` is `~/bin/h` —
so the `dot-` prefix this repository used to carry is gone.

homeshick always creates real directories and symlinks the files inside them,
including nested and hidden ones,
so an existing `~/.config` or `~/bin` is descended into rather than replaced.

## Per-user overrides

homeshick has no per-user or per-machine mechanism:
one castle installs the same files everywhere.
The intended answer is a second castle, which would mean a second repository.

[`personalized/<USER>/`](personalized) therefore stays outside `home/`
and is linked into `~/scriptlets` by [`Makefile`](Makefile) directly,
the way `install-personalized` used to.
This is the one part of the old setup that homeshick does not replace.

## Files

- [`Makefile`](Makefile): `make` links the castle and the per-user overrides,
  `make check` reports what is missing or out of date,
  `make pull` updates the castle and relinks.
- [`bootstrap`](bootstrap): one-shot setup: installs homeshick, clones this castle, runs `make`.

The repository now lives in `~/.homesick/repos/scriptlets` rather than `~/git/scriptlets`;
homeshick looks for castles there and nowhere else.
