...
To install all scripts to `~/bin` (by creating symbolic links), install [GNU Stow](https://www.gnu.org/software/stow/), clone the project and type `make`.
...
Installation is handled by [GNU Stow](https://www.gnu.org/software/stow/).
Stow creates the symbolic links, so there is no generated installer to keep in sync:
adding a file under `stow/` and rerunning `make` is enough.

## Layout

Everything that ends up in the home directory lives in `stow/`,
which is the Stow directory ("`--dir`").
Each of its subdirectories is a Stow *package*:

| Package | Target | Contents |
|---|---|---|
| [`stow/home`](stow/home) | `~` | the bulk of the configuration, plus `bin/` and `git/R/` |
| [`stow/ssh`](stow/ssh) | `~/.ssh` | `config` |
| [`stow/config`](stow/config) | `~/.config` | `diffuse/diffuserc` |
| [`personalized/<USER>`](personalized) | `~/scriptlets` | per-user `gitconfig`, `Rprofile`, `ssh-config` |

Within `stow/home`, a leading `dot-` in a file name maps to a leading `.` in the target,
so `stow/home/dot-bashrc` becomes `~/.bashrc`.
This is Stow's own `--dotfiles` convention, and it matches what this repository used before.

`ssh` and `config` are separate packages rather than `stow/home/dot-ssh` and `stow/home/dot-config`,
because Stow 2.3.x — the version in current Ubuntu — cannot translate a `dot-` prefix on a *directory* name;
it aborts with `stow_contents() called with non-directory path`.
Giving those two trees their own package and an explicit `--target` sidesteps the bug
and keeps the repository usable with the Stow that ships with the distribution.

## Files

- [`.stowrc`](.stowrc): the options shared by all invocations —
  `--dir=stow`, `--target=$HOME`, `--dotfiles`, `--no-folding`.
  Note that `.stowrc` has no comment syntax:
  Stow splits the whole file on whitespace and treats every token as an option.
- [`Makefile`](Makefile): `make` (or `make install`) links everything,
  `make delete` unlinks it again,
  `make restow` does both (use it after renaming or deleting a file, so stale links go away),
  and `make check` prints what `make install` would do without touching the filesystem.
- [`bootstrap`](bootstrap): one-shot setup: clones the repo to `~/git/scriptlets` and runs `make`.

`--no-folding` is deliberate.
Without it, Stow replaces a whole directory with a single symlink when the target does not exist yet,
so a fresh machine would end up with `~/bin -> ~/git/scriptlets/stow/home/bin`
and every file later dropped into `~/bin` would silently land inside the repository.
