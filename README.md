...
To install all scripts to `~/bin`, install [chezmoi](https://www.chezmoi.io/install/), clone the project and type `make`.
...
Installation is handled by [chezmoi](https://www.chezmoi.io/).
There is no generated installer to keep in sync:
chezmoi reads the source directory and makes the home directory match it.

## Layout

[`.chezmoiroot`](.chezmoiroot) points chezmoi at [`home/`](home),
so `README.md`, the `Makefile`, and `obsolete/` stay out of the way.
Inside `home/`, the file name carries the metadata:

| Source | Target | |
|---|---|---|
| `dot_bashrc` | `~/.bashrc` | `dot_` becomes a leading `.` |
| `private_dot_ssh/config` | `~/.ssh/config` | `private_` makes the directory `0700` |
| `bin/executable_h` | `~/bin/h` | `executable_` sets the mode bit |
| `scriptlets/empty_gitconfig.tmpl` | `~/scriptlets/gitconfig` | `.tmpl` is rendered, `empty_` keeps a zero-length result |
| `air.toml` | `~/air.toml` | no prefix, no change |

Entries starting with `.` are chezmoi's own
(and `home/.editorconfig`, which is for this repository and is not installed).

## Per-user overrides

The former `personalized/<USER>/` tree lives in [`home/.chezmoitemplates/personalized/`](home/.chezmoitemplates/personalized),
and the three files under `home/scriptlets/` select from it:

```
{{ includeTemplate (printf "personalized/%s/gitconfig" .chezmoi.username) . }}
```

[`home/.chezmoiignore`](home/.chezmoiignore) is itself a template,
so on a machine whose user has no entry there, `~/scriptlets` is not created at all.

## Symlink mode

[`home/.chezmoi.toml.tmpl`](home/.chezmoi.toml.tmpl) sets `mode = "symlink"`,
so plain configuration files are symlinked back into this repository
and editing `~/.bashrc` still edits `home/dot_bashrc`, as before.

Two kinds of file are exempt, because chezmoi cannot symlink them:

- **executables**, which is all of `~/bin` — they are written as real copies
- **templates**, which is all of `~/scriptlets`

Both need `chezmoi apply` (or `make apply`) after an edit.

## Files

- [`Makefile`](Makefile): `make` runs `chezmoi init --apply` and is safe to rerun;
  `make apply`, `make diff`, `make status` forward to chezmoi;
  `make check` is a dry run.
