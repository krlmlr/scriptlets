...
To install, run the [`bootstrap`](bootstrap) script:

```sh
curl -s https://raw.githubusercontent.com/krlmlr/scriptlets/main/.github/bootstrap | sh
```

Or do it by hand — there is no tool to install first, only Git:

```sh
git clone --bare https://github.com/krlmlr/scriptlets.git ~/.dotfiles.git
...
Its Git directory is a bare repository at `~/.dotfiles.git`,
and its work tree is `$HOME`,
so there is no installation step and nothing to symlink:
checking it out puts every file exactly where it belongs.

There is no dotfile manager here.
[`bin/dotfiles`](bin/dotfiles) is a three-line wrapper around `git`
that supplies `--git-dir` and `--work-tree`:
...
`status.showUntrackedFiles no` is what keeps `dotfiles status` readable —
without it, every file in the home directory shows up as untracked.
...
Plain Git has no per-user mechanism —
...
this README, the [`bootstrap`](bootstrap) script,
[`obsolete/`](obsolete), and the RStudio project file.
