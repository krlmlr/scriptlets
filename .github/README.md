...
To install, install [yadm](https://yadm.io) and clone:

```sh
yadm clone https://github.com/krlmlr/scriptlets.git
```

# Build and install

This repository *is* a home directory.
[yadm](https://yadm.io) keeps its Git directory in `~/.local/share/yadm/repo.git`
and uses `$HOME` as the work tree,
so there is no installation step and nothing to symlink:
checking the repository out puts every file exactly where it belongs.

```sh
yadm status          # like git status, scoped to the tracked files
yadm add ~/.bashrc
yadm commit
yadm push
```
...
The former `personalized/<USER>/` tree becomes yadm *alternates*.
`scriptlets/gitconfig##user.kirill` is checked out on every machine,
but yadm creates the symlink `~/scriptlets/gitconfig` pointing at it
only when the current user is `kirill`.
On an account with no matching alternate, the symlink is simply absent,
which is what `~/.gitconfig`, `~/.Rprofile` and `~/.ssh/config` already tolerate.

`yadm alt` re-evaluates the symlinks;
`yadm clone` and `yadm pull` run it automatically.
The same mechanism keys on `##os.Darwin`, `##distro.ubuntu`, `##hostname.…` and `##class.…`.
...
The repository's own files therefore live out of the way:

- this README is [`.github/README.md`](.github/README.md), which GitHub still renders as the repository front page
- [`.config/yadm/obsolete/`](.config/yadm/obsolete) holds the retired scripts
- [`.config/yadm/scriptlets.Rproj`](.config/yadm/scriptlets.Rproj) is the RStudio project file

`~/.gitignore` now serves two roles at once:
`core.excludesfile` for every other repository, and the ignore file for this one.
...
In the [`.config/yadm/obsolete`](../.config/yadm/obsolete) directory.
